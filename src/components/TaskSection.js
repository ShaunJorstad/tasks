...
class TaskSection extends React.Component {
  ...
  renderSectionTitle() {
    if (!["all", "today", "upcoming"].includes(this.props.selectedList)) {
      return this.props.sectionName !== null ? (
        <input
          className={`col-span-11 text-sfMedium bg-contentBackground text-18 text-${
            this.getList().color
          }`}
          type="text"
          ...
          onChange={(val) => {
            this.props.rootHandlers.editSection(
              this.getList().id,
    ...
    return this.props.sectionName !== null ? (
      <p
        className={`select-none cursor-pointer col-span-11 text-sfMedium text-18 text-${
          this.getList(this.props.listID).color
        }`}
      >
        ...
        <div
          onClick={() => {
            if (this.props.filter === "today") {
              console.log("Functionality unimplemented");
              // this.props.rootHandlers.deleteSection(
              // this.getList().id,
              // this.props.sectionID
              // );
            } else {
              ...
              // this.props.rootHandlers.batchDeleteTasks(
          ...
          className={`sectionDeleteButton mr-4 rounded-full w-5 h-5 border-2 border-gray mt-1 
                    ${
                      this.props.filter === "upcoming"
                        ? null
                        : `hover:bg-${this.getList().color} hover:border-${
                            this.getList().color
                          }   transform hover:scale-75 motion-reduce:transform-none`
  ...
  renderAddTaskButton() {
    return (
      <div
        className={`
                    grid grid-cols-12 py-8 cursor-pointer text-gray hover:text-${
                      this.getList().color
                    }
        ...
        onClick={() => {
          switch (this.props.selectedList) {
            case "all":
              this.props.listHandlers.selectNewTask(
  ...
  getTaskChildren(id) {
    let children = Object.values(this.props.tasks)
  ...
  render() {
    let renderedTasksWithDates = this.props.tasks
      .filter((task) => task.due !== null && task.parentTask === null)
      .sort((a, b) => {
    ...
    let renderedTasksWithoutDates = this.props.tasks
      .filter((task) => task.due === null && task.parentTask === null)
      .sort((a, b) => {
